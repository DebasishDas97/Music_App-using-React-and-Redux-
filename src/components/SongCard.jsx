...
const SongCard = ({ song, i, isPlaying, activeSong, data }) => {
  ...
  if (song.images)
    return (
     ...
     backdrop-blur-sm animate-slideup rounded-md cursor-pointer"
      >
        ...
        <div className="mt-4 flex flex-col">
          <p className="text-gray-100 text-lg truncate">
            <Link to={`/songs/${song?.key}`}>{song.title}</Link>
          </p>
          <p className="text-gray-300 text-opacity-90 text-sm truncate">
            <Link
              to={
                song.artists
